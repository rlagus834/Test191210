...
public class BankMain {
	...
	public static void main(String[] args) {
		...
		/*
		 * 은행프로그램 1.고객등록을할수있다 1.1생성자를 이용한방법 1.2 getter,setter 이용한 방법 2.예금기능을 수행할 수있다
		 * 3.출금기능을 수행할수있다 3.1잔액이0원미만이면 출금 불가능 메세지 출력한다 4.잔액 확인을 할수있다 5.고객정보
		 * -고객번호,이름,계좌번호,잔액 6.고객정보는 ArrayList에저장한다
		 */
		boolean run = true;
			...
			if (1 == input) {
				...
				if (input99 == 1) {
					...
					if (sysinput == 7347) {
						...
						System.out.println("정보가 등록되지않았습니다.");
					}
				} else if (input99 == 2 && count1 >= 1) {
					for (int i = 0; i < list.size(); i++) {
						System.out.println("회원이름:" + list.get(i).getName() + "   보유금액:" + list.get(i).getMoney()
								+ "  계좌번호:" + list.get(i).getNnum() + "  고객번호:" + list.get(i).getCount());
					}
				} else {
				...
				if (list.get(input02).getNnum().equals(input03)) {
					...
					if (1 == input04) {
						...
						System.out.println("송금할 고객의 고객" + "번호와 계좌번호를 입력하세요");
