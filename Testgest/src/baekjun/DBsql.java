	...
	public class DBsql {
		...
		//설명:움직인값에따라 해당유저가 도착한 바뀐 도시위치를 출력하기위해 유저가 도시에 도착했을시 어디도시인지 출력후     (김현) 
		//도착한곳이 무소유지인지,자신의 도시인지,타인의 도시인지,국세청인지,출발지,찬스 분기점인지에 따라 실행되야되는 기능이 다르므로 
		//도착한곳에따라 어디인지 판단해주는 메소드를 생성.
		
		public void CityproPertyCheck(int locationA, String userA,String userB) {//locationA=움직인값 USER A=유저1 USER A=유저2
			String sql = "SELECT CITY,PRICE,PROPERTY FROM MONOPOLY WHERE CITYNO=?";//CITY=도시이름   PRICE=도시가격  PROPERTY=소유권  CITYNO=도시번호  
			try {//전 메소드에서 호출받아온 움직인값을 도시번호에 대입하여 도착한 해당도시와 메소드들에서 이용할 가격,소유권 여부를 알기위한 소유권을 가져옴  
				...
				while (rs.next()) {
					...
					if(locationA==1) { //1번위치는 출발지점이라 변해야할값이 없으므로 리턴  
						...
						UserMoneyTax(saveMoney, userA,userB); //유저가 5번에 위치했을때  메세지출력후 세금메소드 실행
					}
					else if (locationA == 10) { //찬스 IF문
						System.out.println(rs.getString("city")+"에 방문했습니다.");
						Chance(userA);//유저가 10번에 위치했을때  메세지출력후 찬스메소드실행
					}
					else if ((rs.getString("property")).equals(userA)) { //내도시 IF문
						System.out.println("당신의 소유 도시인  "+rs.getString("city")+"에 방문했습니다.");
						return;//자신의 소유지였을땐 변해야할값이 없으므로 리턴
					} 
						...
						UserMoneyPayment(saveMoney, locationA, userA,userB);//상대의 소유지일땐 돈을 지불해야하므로 메세지 출력후 지불메소드 진행
					} 
						...
						case 1:
							CityPurchase(saveMoney, userA,userB,locationA);//무소유지 도시 구매 메소드
							String enter=scan.nextLine(); //nextLINE() 메소드를 사용하는데 이는 Enter전까지 입력받으므로 남아있는 Enter를 소진시키는 용도
							break;
						case 2:
		...
		public void UserMoneyPayment(int price, int locationA, String userA, String userB) {// 상대에서 도시의 가격만큼 값을 지불하는 메소드(김현)
			String sql = "SELECT MONEY FROM USERLIST WHERE NAME=?";  //순서=값을 지불해야하는 유저의 돈정보 출력>1.돈없을경우 파산후 게임종료			
			String city=CitySearch(locationA);                       //                          1.돈있을경우 불러온가격만큼 유저의돈깎임>2.가격보다넘게돈있으면인수여부
			try {                                                                       //                         >2.가격과같거나적은돈있으면상대유저게임진행
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, userA);
				rs = pstmt.executeQuery();
				while (rs.next()) {
