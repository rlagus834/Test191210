package baekjun;
	...
	public class DBsql {
		...
		public void CityproPertyCheck(int location,String userA,String userB) {
			...
			try {  
				if (userA.equals(userA)) { //게임을이용하는 유저는 2명이므로 어느유저가 이용하는지 판별하기위해 IF문을씀
					OtherUser = userB; //상대 유저를 B라고 지정
				} 
				else {
					OtherUser = userA; //반대의 경우는 A라고 지정.
				}
				pstmt = con.prepareStatement(sql);
				...
				while (rs.next()) {
					...
					if(location==1) { //1번위치는 출발지점이라 변해야할값이 없으므로 리턴  
						return;   
					}
					else if (location == 5) { //국세청일땐 세금(지불)만 내게끔 IF문사용 
					...
					else if ((rs.getString("property")).equals(OtherUser)) {// 상대도시 IF문
						System.out.print(rs.getString("city")+"에 방문했습니다.  ");
						System.out.println(rs.getString("property") + "의 소유지 입니다. " + price + "를 지불합니다.");
						UserMoneyPayment(price, location, userA,userB);//상대의 소유지일땐 메세지 출력후 지불 메소드 진행
					} 
						...
						case 1:
							CityPurchase(price, userA,location);//무소유지 도시 구매 메소드
		...
		public void Chance(String user) { //1P,2P가 찬스 지역에 위치하는 실행되는 메소드,랜덤한 확률에 의해 실행.(전석종)
