...
public class DBsql {
	...
	int saveMoney = 0,saveMoney2 = 0,saveMoney3 =0;
	...
	public void TurnRepeat() throws IOException { // 1번 커맨드 누르면 실행
		System.out.println("--------------------------------------");
		System.out.println("출발지            | 인천          | 대전           |부산                 ");
		System.out.println("--------------------------------------");
		System.out.println("서울               |                 | 국세청            ");
		System.out.println("----------                 -----------");
		System.out.println("수원               |                 | 제주도            ");
		System.out.println("---------------------------------------");
		System.out.println("  대전              |  대구        | 광주           |  독도             ");
		System.out.println("---------------------------------------");
		...
		String userB="B";
		for (;;) {
			locationA = TurnA(locationA);
			if(!moneySelectss(userA,saveMoney2)) {
				break;
			}
			locationB = TurnB(locationB);
			if(!moneySelectss(userB,saveMoney2)) {
				break;
			}
			}
	...
	public int TurnA(int locationA) throws IOException {
		String name = "A";
		...
		if (locationA >= 13) { // 현재위치가 13보다 크거나 같아지는 경우
			...
			proPertyCheck(locationA, "A"); // @소유지권한체크메소드
			return locationA;
		} else {
			System.out.println(dice + "칸 앞으로!!" + name + "의 위치는 " + locationA + "\n");
			proPertyCheck(locationA, "A");// @소유지권한체크메소드
	...
	public int TurnB(int locationB) throws IOException {
		String othername = "B";
		...
		if (locationB >= 13) {
			...
			proPertyCheck(locationB, "B");// @소유지권한체크메소드
			return locationB;
		} else {
			System.out.println(dice + "칸 앞으로!!" + othername + "의 위치는 " + locationB + "\n");
			proPertyCheck(locationB, "B");// @소유지권한체크메소드
	...
	public void proPertyCheck(int dice, String user) {// 도시 소유자 확인 (주사위/계정이름)
	...
	public void payment(int price, String user) {// 지불 메소드(가격,지불하는계정)
		String sql = "SELECT MONEY FROM USERLIST WHERE NAME=?";
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, user);
...
//게임진행중에  보유도시, 보유잔액 보려고만듬 
	...
	public void tax(int price, String user) {// 세금 메소드(가격,지불하는계정)
		String sql = "SELECT MONEY FROM USERLIST WHERE NAME=?";
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, user);
	...
	public boolean moneySelectss(String user,int price) {
		String sql = "SELECT MONEY FROM USERLIST WHERE NAME=?";
		try {// 잔액보유
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, user);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				saveMoney=rs.getInt("money")-price;
				System.out.println(user + "님의 잔액: " + saveMoney);
				if(saveMoney<=0) {
